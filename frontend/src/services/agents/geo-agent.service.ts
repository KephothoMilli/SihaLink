...
export class GeoAgentService {
    ...
    */
     ...
     */
    async findNearestFacilities(location: Location, radius_km: number = 50): Promise<Facility[]> {
     ...
     */
    async getETAToFacility(from: Location, to: Location): Promise<{ distance_km: number; eta_minutes: number }> {
    ...
    async healthCheck(): Promise<boolean> {
        try {
            return await this.api.get('/health/geo');
        } catch {
            return false;
        }
    }
